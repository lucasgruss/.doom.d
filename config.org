...
Ansi-term and vterm are pretty decent, but EXWM makes it possible to use an
/actual/ terminal emulator, which usually works better on corner cases.
...
(defun my/invoke-x-terminal-emulator ()
  ...
  (interactive)
  (if (string= (buffer-name) exwm-x-terminal-emulator)
      (bury-buffer)
    (if (get-buffer exwm-x-terminal-emulator)
        (exwm-workspace-switch-to-buffer exwm-x-terminal-emulator)
